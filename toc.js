    ...
    const html =
      ...
      Array.prototype.map
        .call(headings, function (heading, index) {
          ...
          const safeHID = heading.id;
          const anchorId = `anchor-${safeHID}`;

          // Generate the HTML
          html += `<li><a id='${anchorId}' href='#${safeHID}'>${heading.innerHTML.trim()}</a></li>`;

          $(heading).append(
            `<a title="Go back" style="font-size: 1.2rem; margin-left: 0.5rem; top: -0.2rem; position: relative;" href="#${anchorId}"><i class="fas fa-level-up-alt"/></a>`
          );
